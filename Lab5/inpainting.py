...
# mask_b: iteration decoding initial mask, where mask_b is true means mask
    def inpainting(self,image,mask_b,i): #MakGIT inference
        ...
        self.model.eval()
        with torch.no_grad():
            zq, z_indices = self.model.encode_to_z(image) #z_indices: masked tokens (b,16*16)
            ...
            for step in range(self.total_iter):
                if step == self.sweet_spot:
                    break
                ratio = (step+1)/args.total_iter #this should be updated # step+1 then the last ratio can be 1.0

                if self.mask_func == 'linear':
                    ratio = step / self.total_iter
                elif self.mask_func == 'cosine':
                    #a = torch.pi * step / self.total_iter
                    #print(torch.pi)
                    ratio = 1-0.5 * (1 + math.cos(torch.pi * step / self.total_iter))
                elif self.mask_func == 'square':
                    ratio = (step / self.total_iter) ** 2
                else:
                    raise NotImplementedError("Mask function mode not supported.")

                z_indices_predict, mask_bc = self.model.inpainting(z_indices, mask_bc,mask_num, ratio,pre_ratio)
                pre_ratio = ratio
    ...
    #         z_indices = z_indices.view(image.size(0), -1)  # reshape to (b, 16*16)
    #         mask_b = mask_b.to(device=self.device)
    #         mask_bc = mask_b.clone()
            
    #         z_indices[mask_b] = self.mask_token_id

    #         gamma = self.model.gamma_func(self.mask_func)
    #         """
    #         _, z_indices = self.model.encode_to_z(image)
    #         mask_num = mask_b.sum().item()  # 遮罩 token 的總數
    #         z_indices_predict = z_indices
    #         mask_bc = mask_b
    #         mask_b = mask_b.to(device=self.device)
    #         mask_bc = mask_bc.to(device=self.device)
    ...
    #                 #a = torch.pi * step / self.total_iter
    #                 #print(torch.pi)
    #                 ratio = 0.5 * (1 + math.cos(torch.pi * step / self.total_iter))
    #             elif self.mask_func == 'square':
    #                 ratio = (step / self.total_iter) ** 2
    #             else:
    #                 raise NotImplementedError("Mask function mode not supported.")
    #             # Call the inpainting function from the model
    #             z_indices_predict, mask_bc = self.model.inpainting(z_indices, mask_bc, mask_num, 1-ratio) # [1, 256]
...
#TODO3 step1-2: modify the path, MVTM parameters
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='./transformer_checkpoints/transformer_epoch_70.pt', help='load ckpt')
    #parser.add_argument('--load-transformer-ckpt-path', type=str, default='./check/transformer_epoch_55.pt', help='load ckpt')
